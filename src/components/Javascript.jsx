...
const Javascript = () => {
  ...
  const EasyTestCompleted = localStorage.getItem(`${userPrefix}easyjstestCompleted`) === "true";
  const MediumTestCompleted = localStorage.getItem(`${userPrefix}mediumjstestCompleted`) === "true";
  const HardTestCompleted = localStorage.getItem(`${userPrefix}hardjstestCompleted`) === "true";
  ...
  const handleEasyJs = () => {
    ...
    localStorage.removeItem(`${userPrefix}easyjstestCompleted`, "true");
    
   
  };

  const handleMediumJs = () => {
    localStorage.removeItem(`${userPrefix}timeLeft`);
    setTimeLeft(60); // Reset the timer state to 60 seconds
    ...
    localStorage.removeItem(`${userPrefix}mediumjstestCompleted`, "true");
  ...
  const handleHardJs = () => {
    ...
    localStorage.removeItem(`${userPrefix}hardjstestCompleted`, "true");
    ...
    <div className="min-h-screen flex items-center justify-center bg-cover bg-center bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
      <div className="lg:h-[80vh] lg:w-[90vw] w-[95vw] relative flex flex-col items-center gap-12 justify-center rounded-lg shadow-lg bg-black bg-opacity-30 backdrop-filter backdrop-blur-lg p-8">
        ...
        <div className="flex flex-col md:flex-row gap-8 justify-center items-center">
          ...
          <Link to="/javascript/easyjs" onClick={handleEasyJs}>
          ...
          <Link to="/javascript/mediumjs" onClick={handleMediumJs}>
          ...
          {/* {showSolutionLink && ( */}
          {MediumTestCompleted ? (
          <Link to="/javascript/mediumjssols" onClick={handleJSSol}>
          <div className="w-64 h-40 border-4 border-yellow-600 rounded-lg p-6 flex flex-col justify-center items-center transition-transform transform hover:scale-105 hover:bg-yellow-200 group">
          ...
          <Link to="/javascript/hardjs" onClick={handleHardJs}>
          ...
          {/* {showSolutionLink && ( */}
          {HardTestCompleted ? (
          <Link to="/javascript/hardjssols" onClick={handleJSSol}>
          ...
          </Link> ) : (<div className="w-64 h-40 border-4 border-gray-500 rounded-lg p-6 flex flex-col justify-center items-center opacity-50 cursor-not-allowed">
